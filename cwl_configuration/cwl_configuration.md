...
添加VSCode到环境变量中。D:\Microsoft VS Code\bin
...
另一种解决方法：使用CMake构建
1.安装CMake并添加环境变量
2.项目文件夹内新建CMakeLists.txt 并编写，VSCode识别到CMakeLists.txt后 选择环境G++/MSVC（或后续F7/生成再选择） 并自动创建build文件夹
3.底部的生成按键：构建exe文件（即相当于g++ test.cpp -o test.exe）
底部的DeBug（F5）
底部的启动但不调试（相当于Ctrl+F5）

调试时依旧可以利用launch.json，设置使用外部控制台or内部控制台等等

另：使用MSVC编译器
删除之前使用g++编译器生成的build文件，对CMakeLists.txt新增一些MSVC的额外配置
2.Ctrl+Shift+P 输入 CMake: Delete Cache and Reconfigure，清除旧的 MinGW 缓存
3.Ctrl+Shift+P 输入 CMake: Select a Kit,在弹出的列表里，选择 MSVC 编译器
4.F7（选择工具包）编译或者底部生成 之后选择 直接运行 或者 调试运行

## C++MSVC环境配置（CPP_MSVC文件夹）

使用Devoloper PowerShell for VS命令行，敲入code命令启动VSCode，这样会添加VS的一些环境变量。

// ×直接使用VSCode打开文件后：
//  F5选择调试器C++（Windows） 选择配置C/C++:(Windows)启动生成launch.json 再次F5报错：MSVC编译器不存在

因此通过Devoloper PowerShell for VS命令行的方式启动VSCode：
F5选择调试器C++（Windows）选择配置C/C++:cl.exe构建和调试活动文件 生成task.json、exe、obj等文件 F5成功运行。
单文件为上述配置，多文件则使用CMake。

## 静态库与动态库

静态库lib 动态库dll
将一些函数让用户调用的同时，隐藏其实现细节

## QtVSCode开发

1.安装Qt CPP官方插件，将D:\Qt\6.11.0\mingw_64\bin加入系统环境变量
2.用QtCreator创建一个项目，用VSCode打开该项目

发布：
1.将Debug模式转为Release模式
2.创建新文件夹，将exe程序放进去
3.在exe所在文件夹cmd输入 windeployqt QtVSCodeDemo.exe以打包必要的dll文件等
4.后续压缩/做成安装包/单文件打包等方式，发布。
